...
from hiku.directives import DirectiveInfo, Location, SchemaDirective, DirectiveField, get_fields, wrap_dataclass
...
@dataclass
class FederationSchemaDirectiveInfo(DirectiveInfo):
...
def schema_directive(
    *,
    ...
    :param compose: If True, the directive will be added to the schema
        ...
        See: https://www.apollographql.com/docs/federation/federated-types/federated-directives#composedirective
    :param import_url: The URL of the subgraph that defines the directive.
        See: https://specs.apollo.dev/link/v1.0/#@link.url
    """
    def _wrap(cls: T) -> T:
        if not issubclass(cls, FederationSchemaDirective):
            raise TypeError(f'{cls} is not a subclass of {FederationSchemaDirective}')
...
class FederationSchemaDirective(SchemaDirective):
    ...
    __directive_info__: FederationSchemaDirectiveInfo
...
class Key(FederationSchemaDirective):
    ...
    resolvable: bool = DirectiveField(
        'resolvable',
...
class Link(FederationSchemaDirective):
    ...
    """
    url: str = DirectiveField(
        'url',
        type_ident=NON_NULL(SCALAR("String")),
    )
    as_: Optional[str] = DirectiveField(
        'as',
        type_ident=SCALAR("String"),
        default_value=None,
    )
    # EXECUTION or SECURITY
    for_: Optional[str] = DirectiveField(
        'for',
        type_ident=SCALAR("link__Purpose"), # TODO(mkind): this type ???
        default_value=None,
    )
    import_: Optional[List[str]] = DirectiveField(
        'import',
        type_ident=LIST(SCALAR("[link__Import]")), # TODO(mkind): this type ??? list(scalar) may be link__Import not [Link__Import
        default_value=None,
    )
