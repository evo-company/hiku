...
RUN uv export --format requirements.txt --no-dev --output-file requirements-base.txt && \
  uv pip install --system -r requirements-base.txt --no-deps --no-cache-dir --index-strategy unsafe-best-match

FROM base AS dev

RUN uv export --format requirements.txt --no-default-groups --group dev --output-file requirements-dev.txt && \
  uv pip install --system -r requirements-dev.txt --no-deps --no-cache-dir --index-strategy unsafe-best-match

FROM base AS test

RUN uv export --format requirements.txt --no-default-groups --group dev --group test --output-file requirements-test.txt && \
  uv pip install --system -r requirements-test.txt --no-deps --no-cache-dir --index-strategy unsafe-best-match

FROM base AS docs

RUN uv export --format requirements.txt --no-default-groups --group dev --group docs --output-file requirements-docs.txt && \
