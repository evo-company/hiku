...
from tests.test_federation.utils import (
    GRAPH,
    ASYNC_GRAPH, async_direct_link, direct_link, id_resolver, resolve_reference_direct,
)
...
async def test_multiple_types_in_representations_async():
    ...
    graph = Graph([
        ...
        Root([]),
    ], unions=[Union('Media', ['Audio', 'Video'])], is_async=True)
    ...
    result = await schema.execute(query['query'], query['variables'], query['operationName'])
